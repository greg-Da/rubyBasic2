...
def size_list
    puts "il y a #{$array.length()} handles"
    perform
end
...
def number_of_size
    newHash = {}
    $array.each do |elem|
        if newHash.has_key?(elem.length - 1)
            newHash[elem.length - 1] += 1
        else
            newHash[elem.length - 1] = 1
        end
    end
    puts newHash.sort.to_h
    perform
end
...
def perform
    ...
    puts '3 - Nombre de handle avec 5 char'
    puts '4 - Nombre de handle avec une majuscule'
    puts '5 - Liste par ordre aplhabetique'
    puts '6 - Liste par ordre de taille'
    puts '7 - Index de @epenser'
    ...
    when 9 
        ...
        puts "Ne fais pas partie des propositions"
        perform
    end
end
perform
